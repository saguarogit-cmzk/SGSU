...
const clients=(o.clients||[]).map(c=>`<tr><td>${e(c.name)}</td><td>${validDate(c.expiresAt)?new Date(c.expiresAt).toLocaleDateString('hr-HR'):'—'}</td><td class="rowacts"><button class="ovDel danger" data-n="${e(c.name)}">Opozovi</button></td></tr>`).join('');
