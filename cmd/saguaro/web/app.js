...
const rows=cfg.uplinks.length?cfg.uplinks.map((u,i)=>{const reorder=fo?`${iconBtn('up','Prioritet gore','muwUp',`data-i="${i}"${i===0?' disabled':''}`)}${iconBtn('down','Prioritet dolje','muwDown',`data-i="${i}"${i===cfg.uplinks.length-1?' disabled':''}`)}`:'';return `<tr><td><b>${e(u.name)}</b></td><td class="muted">${e(nlabel(u.interface))}</td><td class="muted">${e(u.gateway)}</td><td>${roleBadge(i)}</td><td class="muted">${e(u.healthCheck||'—')}</td><td class="rowacts">${reorder}${iconBtn('del','Ukloni','danger muwDel',`data-i="${i}"`)}</td></tr>`}).join(''):'<tr><td colspan="6" class="muted">Nema uplinkova — dodaj barem dva.</td></tr>';
