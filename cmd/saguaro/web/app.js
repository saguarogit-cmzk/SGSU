...
{title:'Podaci',render:s=>s.type==='public'?`<label>Ime certifikata <input id="c2name" value="${escapeHtml(s.name||'')}" placeholder="wiki"></label><label>Javna domena (FQDN) <input id="c2dom" value="${escapeHtml(s.domain||'')}" placeholder="wiki.example.com"></label><label>Kontakt e-mail (ACME) <input id="c2mail" value="${escapeHtml(s.email||'')}" placeholder="admin@example.com"></label><p class="muted">Domena mora biti javno dostupna na portu 80 tijekom izdavanja (bez wildcard-a).</p>`:`<label>Ime certifikata <input id="c2name" value="${escapeHtml(s.name||'')}" placeholder="wiki"></label><label>SAN-ovi (zarezom: imena i/ili IP) <input id="c2sans" value="${escapeHtml(s.sans||'')}" placeholder="wiki.example.internal, 192.168.10.5"></label>`,collect:s=>{s.name=$('#c2name').value.trim().toLowerCase();if(s.type==='public'){s.domain=$('#c2dom').value.trim().toLowerCase();s.email=$('#c2mail').value.trim();return(s.name&&s.domain&&s.email)?null:'Unesite ime, domenu i e-mail.'}s.sans=$('#c2sans').value.trim();return(s.name&&s.sans)?null:'Unesite ime i barem jedan SAN.'}},
...
{title:'Pregled i izdavanje',render:s=>`<p>Tip: <b>${s.type==='public'?'Javni (Let\'s Encrypt)':'Interni'}</b><br>Ime: <b>${escapeHtml(s.name)}</b><br>${s.type==='public'?'Domena: <b>'+escapeHtml(s.domain)+'</b>':'SAN-ovi: <b>'+escapeHtml(s.sans)+'</b>'}<br>Deploy: <b>${s.gui?'GUI vhost':'samo izdaj'}</b></p>`}],
async s=>{const body=s.type==='public'?{type:'public',name:s.name,domain:s.domain,email:s.email,deployGui:s.gui}:{type:'internal',name:s.name,sans:s.sans.split(',').map(x=>x.trim()).filter(Boolean),deployGui:s.gui};const r=await api('/api/certs/issue',{method:'POST',body:JSON.stringify(body)});if(r.note)alert(r.note);openModule('certificates')})}
