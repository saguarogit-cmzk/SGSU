...
const UTM_MODULES=new Set(['gateway','fwrules','ids','webproxy','vpn','openvpn','sitevpn','ipsec','multiwan']);
...
const clients=(o.clients||[]).map(c=>`<tr><td>${e(c.name)}</td><td>${c.expiresAt?new Date(c.expiresAt).toLocaleDateString():'—'}</td><td class="rowacts"><button class="ovDel danger" data-n="${e(c.name)}">Opozovi</button></td></tr>`).join('');
...
<div class="panel"><h2>Korisnici (${(o.clients||[]).length})</h2><table><thead><tr><th>Korisnik</th><th>Vrijedi do</th><th></th></tr></thead><tbody>${clients||'<tr><td colspan="3" class="muted">Nema korisnika.</td></tr>'}</tbody></table>
${o.provisioned?`<h3>Dodaj korisnika</h3><div class="filterbar"><input id="ovcName" placeholder="ime (npr. ivan)"><input id="ovcExp" type="number" placeholder="dana (0 = 2 god)" style="max-width:150px"><button type="button" id="ovcAdd">Dodaj i preuzmi .ovpn</button></div><div id="ovcMsg" class="muted"></div>`:'<p class="muted">Uključi OpenVPN da bi mogao dodavati korisnike.</p>'}</div>`;
...
const ovcAdd=$('#ovcAdd');if(ovcAdd)ovcAdd.onclick=async()=>{const name=$('#ovcName').value.trim();const exp=parseInt($('#ovcExp').value,10)||0;const m=$('#ovcMsg');if(!name){m.textContent='Upiši ime korisnika.';return}m.textContent='Generiram profil…';try{const res=await fetch('/api/openvpn/clients',{method:'POST',headers:{'Content-Type':'application/json','X-CSRF-Token':csrfToken()},body:JSON.stringify({name,expiryDays:exp})});if(!res.ok){const b=await res.json().catch(()=>({}));throw Error(b.error||'Greška')}const blob=await res.blob();const url=URL.createObjectURL(blob);const aTag=document.createElement('a');aTag.href=url;aTag.download=name+'.ovpn';aTag.click();URL.revokeObjectURL(url);m.textContent='Profil '+name+'.ovpn preuzet.';openvpnPage()}catch(err){m.textContent=err.message}};}
