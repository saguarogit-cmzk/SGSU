...
const accSummary=a=>!a||!a.length?'<span class="muted">sve interno</span>':a.map(r=>e(r.dest)+(r.port?':'+r.port:'')+(r.proto&&r.proto!=='any'?'/'+r.proto:'')).join(', ');
...
<table class="compact"><thead><tr><th>Odredište</th><th>Servis</th><th>Proto/Port</th><th></th></tr></thead><tbody>${rules.length?rules.map((r,i)=>`<tr><td><b>${e(r.dest)}</b></td><td>${e(svcName(r))}</td><td class="muted">${r.proto==='any'?'sve':e(r.proto)+(r.port?':'+r.port:'')}</td><td class="rowacts"><button class="oaDel danger" data-i="${i}">Ukloni</button></td></tr>`).join(''):'<tr><td colspan="4" class="muted">Bez ograničenja — smije na sve interno.</td></tr>'}</tbody></table>
<div class="filterbar"><input id="oaDest" placeholder="odredište (npr. 10.10.10.5 ili 10.10.10.0/24)"><select id="oaSvc">${OVSVC.map(s=>`<option value="${s[0]}">${e(s[1])}</option>`).join('')}</select><input id="oaPort" type="number" placeholder="ili custom port" style="max-width:140px"><button type="button" id="oaAdd" class="ghost">Dodaj pravilo</button></div>
...
$('#oaAdd').onclick=()=>{const dest=$('#oaDest').value.trim();const sv=OVSVC.find(s=>s[0]===$('#oaSvc').value);let proto='any',port=0;if(sv&&sv[0]&&sv[0]!=='all'){proto=sv[2];port=sv[3];}const cp=parseInt($('#oaPort').value,10)||0;if(cp){port=cp;if(proto==='any')proto='tcp';}if(!isIPv4(dest)&&!isCIDR(dest)){$('#oaMsg').textContent='Odredište mora biti IP ili CIDR.';return}rules.push({dest,proto,port});$('#oaDest').value='';$('#oaPort').value='';render()};
