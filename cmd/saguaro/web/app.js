...
<div class="panel"><h2>Čarobnjaci</h2><div class="wizRow"><button id="wzNet">Postavi DHCP mrežu</button> <button id="wzRes">Dodaj DHCP rezervaciju</button> <button id="wzZone">Dodaj DNS zonu</button> <button id="wzGw">Postavi gateway (WAN/LAN)</button> <button id="wzMail">Postavi mail alarme</button></div></div></div>
...
// Netplan applies (WAN adrese, VLAN pod-sučelja) same se vraćaju nakon 120 s bez
// potvrde — isto kao firewall — pa svaki apply ostavlja traku za potvrdu/vraćanje.
function netConfirmBar(kind,el,label){const P=kind==='wan'?{c:'/api/wan/confirm',r:'/api/wan/rollback'}:{c:'/api/firewall/zones/vlans-confirm',r:'/api/firewall/zones/vlans-rollback'};
el.innerHTML=`${escapeHtml(label)} <b>Potvrdi unutar 120 s</b> ili se vraća prethodna konfiguracija. <button data-nc="c">Potvrdi (zadrži)</button> <button data-nc="r" class="ghost">Vrati odmah</button>`;
el.querySelector('[data-nc="c"]').onclick=async()=>{try{await api(P.c,{method:'POST',body:'{}'});el.textContent='Potvrđeno — konfiguracija je zadržana.'}catch(err){el.textContent=err.message}};
el.querySelector('[data-nc="r"]').onclick=async()=>{try{await api(P.r,{method:'POST',body:'{}'});el.textContent='Vraćena je prethodna konfiguracija.'}catch(err){el.textContent=err.message}}}
// Prozor nadživi reload stranice, pa se stanje dohvaća i pri otvaranju modula.
async function netPendingCheck(kind,sel,label){try{const p=await api('/api/net/pending');const el=$(sel);if(el&&p[kind])netConfirmBar(kind,el,label)}catch(err){}}
...
${help('<b>Portovi:</b> odaberi <b>WAN port</b> (prema internetu) i <b>LAN port</b> (prema klijentima). Ne znaš koji je koji fizički? Otvori <b>Mreža → Interfaces</b> i klikni Identificiraj (LED zatreperi). <b>LAN / klijentska mreža</b> je subnet koji uređaj poslužuje (DHCP/DNS) — sam se popuni iz LAN porta. <b>Gateway mod</b> uključuje routing WAN↔LAN, <b>NAT</b> pušta klijente na internet preko WAN adrese. <b>Pristup upravljanju</b>: biraš odgovara li uređaj na SSH/GUI s LAN i/ili WAN strane; barem jedan mora ostati. <b>Port forward</b> (napredno) otvara vanjski port prema unutarnjem poslužitelju. Primjena traži potvrdu unutar 120 s — ako izgubiš pristup, vraća se stara konfiguracija.')}
...
${toggle('gwBrute',c.bruteForceProtect,'Zaštita od pogađanja lozinki (ograniči nove veze prema SSH-u i GUI-ju po izvorišnoj adresi)')}
...
wireTabs('gwTabs');
const payload=()=>({adminNetwork:$('#gwAdmin').value.trim(),clientNetwork:$('#gwClient').value.trim(),dhcpInterface:$('#gwDhcpIf').value.trim(),gatewayEnabled:$('#gwEnabled').checked,wanInterface:$('#gwWan').value,lanInterface:$('#gwLan').value,natEnabled:$('#gwNat').checked,mgmtOnLan:$('#gwMgmtLan').checked,mgmtOnWan:$('#gwMgmtWan').checked,hairpinNat:$('#gwHairpin').checked,bruteForceProtect:$('#gwBrute').checked,portForwards:pf,snatRules:snat,nat11:nat11});
...
if(nics.length<2){alert('Za gateway trebaju najmanje 2 mrežna sučelja (WAN + LAN). Pronađeno: '+nics.length);return}
const opt=(v,ex)=>nics.filter(n=>n.name!==ex).map(n=>`<option ${n.name===v?'selected':''}>${escapeHtml(n.name)}</option>`).join('');
runWizard('Postavljanje gatewaya (WAN / LAN / NAT)',[
...
// API tokeni: bearer vjerodajnice za automatizaciju. Tajna se prikazuje samo
// jednom, pri stvaranju — poslije se može još samo opozvati.
let toks=null;try{toks=(await api('/api/tokens')).tokens}catch(e){toks=null}
const tokRows=(toks||[]).map(t=>`<tr><td><b>${escapeHtml(t.name)}</b><br><span class="muted">${escapeHtml(t.preview)}…</span></td><td>${escapeHtml(t.role)}</td><td>${t.expiresAt?new Date(t.expiresAt).toLocaleDateString():'bez isteka'}${t.expired?' <span class="status st-error">istekao</span>':''}</td><td>${t.lastUsed?new Date(t.lastUsed).toLocaleString():'—'}</td><td><button class="tkDel" data-id="${escapeHtml(t.id)}" data-n="${escapeHtml(t.name)}">Opozovi</button></td></tr>`).join('');
const tokens=toks===null?'':`<div class="panel"><h2>API tokeni (${toks.length})</h2>
${help('Token služi da skripta, Ansible ili nadzorni sustav zovu API bez korisničke prijave. Šalje se kao zaglavlje <b>Authorization: Bearer &lt;token&gt;</b> i tada nije potreban CSRF.<br><br>Token nosi <b>ulogu</b> — za nadzor uzmi <i>read-only</i>, za promjene onoliko koliko posao stvarno treba. Tajna se prikazuje <b>samo jednom</b>; ako se izgubi, opozovi token i napravi novi. Token ne može stvarati ni brisati druge tokene, pa procurjela vjerodajnica ne može sama sebi napraviti zamjenu.')}
${toks.length?`<table><thead><tr><th>Naziv</th><th>Uloga</th><th>Istek</th><th>Zadnja upotreba</th><th></th></tr></thead><tbody>${tokRows}</tbody></table>`:'<p class="muted">Nema tokena.</p>'}
<form id="tkAdd" class="stack"><h3>Novi token</h3><label>Naziv (npr. ansible, zabbix) <input id="tkName" required></label><label>Uloga <select id="tkRole">${ROLES.map(r=>`<option>${r}</option>`).join('')}</select></label><label>Trajanje u danima (0 = bez isteka) <input id="tkDays" type="number" min="0" max="3650" value="0"></label><div><button type="submit">Stvori</button></div><div id="tkMsg" class="muted"></div></form></div>`;
$('#content').innerHTML=mine+admin+tokens;
if($('#tkAdd')){$('#tkAdd').onsubmit=async e=>{e.preventDefault();const m=$('#tkMsg');m.textContent='';try{const r=await api('/api/tokens',{method:'POST',body:JSON.stringify({name:$('#tkName').value.trim(),role:$('#tkRole').value,days:parseInt($('#tkDays').value||'0',10)})});
prompt('Token — zapiši ga sada, više se neće prikazati:',r.secret);usersPage()}catch(err){m.textContent=err.message}};
document.querySelectorAll('.tkDel').forEach(b=>b.onclick=async()=>{if(!confirm(`Opozvati token "${b.dataset.n}"? Sve što ga koristi odmah gubi pristup.`))return;try{await api(`/api/tokens/${encodeURIComponent(b.dataset.id)}`,{method:'DELETE'});usersPage()}catch(err){alert(err.message)}})}
...
async function backupPage(){const b=await api('/api/backup');const bkFiles=(await api('/api/backup/files').catch(()=>({files:[]}))).files||[];
const isMainArchive=n=>/^saguaro-\d{8}T[0-9A-Z]+\.tar\.gz\.age$/.test(n);
const filesRows=bkFiles.length?bkFiles.filter(f=>/\.age$/.test(f.name)).map(f=>`<tr><td><code>${escapeHtml(f.name)}</code></td><td class="muted" style="font-variant-numeric:tabular-nums">${fmtBytes(f.size)}</td><td class="muted small">${new Date(f.time).toLocaleString('hr-HR')}</td><td class="rowacts"><a href="/api/backup/download/${encodeURIComponent(f.name)}" download style="display:inline-flex;align-items:center;gap:6px;padding:7px 13px;border-radius:var(--radius-sm);background:var(--brand);color:var(--brand-ink);font-weight:600;font-size:13px;text-decoration:none">⭳ Preuzmi</a>${isMainArchive(f.name)?` <button class="bkRestore danger" data-n="${escapeHtml(f.name)}" title="Vrati konfiguraciju i baze iz ove kopije (za oporavak uređaja)">Vrati iz ove kopije</button>`:''}</td></tr>`).join(''):'<tr><td colspan="4" class="muted">Još nema lokalnih kopija — klikni „Pokreni backup sada".</td></tr>';
const filesPanel=`<div class="panel"><h2>Lokalne kopije <span class="badge">age-šifrirano</span></h2>
<p class="muted small">Arhive spremljene lokalno (<code>/var/backups/saguaro</code>). Svaka je <b>age-šifrirana</b> — <b>preuzmi</b> je i čuvaj izvan uređaja. <b>Vrati iz ove kopije</b> je za oporavak nakon kvara: prepiše konfiguraciju i baze i restarta servise. <br><b>DR komplet</b> = arhiva <u>plus</u> ključ <code>/etc/saguaro/backup.agekey</code> (spremi ga offline pri instalaciji!) — bez ključa se šifrirani backup ne može otvoriti. Tok: svježi Ubuntu → instaliraj SNA → vrati svoj ključ na uređaj → stavi arhivu → Vrati. <code>saguaro-ca-*</code> je zaseban backup internog CA.</p>
<table><thead><tr><th>Datoteka</th><th>Veličina</th><th>Nastala</th><th></th></tr></thead><tbody>${filesRows}</tbody></table></div>`;
const keyPanel=`<div class="panel"><h2>Ključ za oporavak (DR) <span class="badge">osjetljivo</span></h2>
<p class="muted small">Ovaj <b>age privatni ključ</b> dešifrira <b>sve</b> tvoje backupe. <b>Preuzmi ga i čuvaj offline</b> — kao master lozinku, <b>odvojeno</b> od arhiva; tko ima i ključ i arhivu, ima i podatke. Bez ključa se backup <b>ne može</b> vratiti. Na <b>novom uređaju</b> nakon instalacije prvo <b>uvezi</b> ovaj ključ, pa onda „Vrati iz ove kopije".</p>
<div class="btnrow"><a href="/api/backup/key" download style="display:inline-flex;align-items:center;gap:6px;padding:7px 13px;border-radius:var(--radius-sm);background:var(--brand);color:var(--brand-ink);font-weight:600;font-size:13px;text-decoration:none">⭳ Preuzmi ključ</a></div>
<label style="display:block;margin-top:.7rem">Uvezi ključ (oporavak na novom uređaju)<textarea id="bkKeyIn" rows="3" placeholder="AGE-SECRET-KEY-1…" style="width:100%;font-family:monospace;font-size:12px;margin-top:.3rem"></textarea></label>
<div class="btnrow"><button type="button" id="bkKeyImport" class="danger">Uvezi ključ</button></div><div id="bkKeyMsg" class="muted"></div></div>`;
const drill=b.restoreDrillDue?`<div class="panel error"><h2>⚠ Restore drill</h2><p>Provjera povrata podataka nije izvedena u zadnjih ${b.restoreDrillDays} dana (${b.lastDrill&&!b.lastDrill.startsWith('0001')?'zadnja: '+new Date(b.lastDrill).toLocaleDateString():'nikad'}). Napravite testni restore pa potvrdite.</p><button id="bkVerify">Provjeri kopiju (stvarna provjera)</button> <button id="bkDrill" class="ghost">Označi drill izvedenim</button></div>`:`<div class="panel"><p class="muted">Restore drill zadnji put: ${b.lastDrill&&!b.lastDrill.startsWith('0001')?new Date(b.lastDrill).toLocaleDateString():'—'} (podsjetnik svakih ${b.restoreDrillDays} dana). <button id="bkVerify">Provjeri kopiju (stvarna provjera)</button> <button id="bkDrill" class="ghost">Označi drill izvedenim</button></p></div>`;
...
<div class="btnrow"><button type="submit">Spremi</button> <button type="button" id="bkRun">Pokreni backup sada</button>${b.scheduleOff?'':' <button type="button" id="bkDisable" class="ghost">Isključi raspored</button>'}</div>
<p class="muted small">${b.scheduleOff?'⏸ Zakazani backup je <b>isključen</b> — radi samo ručni „Pokreni backup sada". Klik na <b>Spremi</b> ponovno uključuje raspored.':'▶ Zakazani backup je <b>aktivan</b> (raspored: '+escapeHtml(b.schedule||'daily')+').'}</p>
<p class="muted">Šifriranje je obavezno (age); off-site arhive su već šifrirane pa transport samo mora biti autenticiran.</p><div id="bkMsg" class="muted"></div></form></div>${filesPanel}${keyPanel}`;
...
if($('#bkVerify'))$('#bkVerify').onclick=async()=>{const b=$('#bkVerify');b.disabled=true;const old=b.textContent;b.textContent='Provjeravam…';try{const r=await api('/api/backup/verify',{method:'POST',body:'{}'});alert(r.note||'Kopija je provjerena.');backupPage()}catch(e){alert(e.message);b.disabled=false;b.textContent=old}};
...
const bkDis=$('#bkDisable');if(bkDis)bkDis.onclick=async()=>{if(!confirm('Isključiti zakazani backup? On-demand „Pokreni backup sada" i dalje radi; „Spremi" ga ponovno uključuje.'))return;$('#bkMsg').textContent='Isključujem raspored…';try{await api('/api/backup/disable',{method:'POST',body:'{}'});backupPage()}catch(e){$('#bkMsg').textContent=e.message}};
document.querySelectorAll('.bkRestore').forEach(el=>el.onclick=async()=>{const n=el.dataset.n;if(prompt('⚠ OPREZ: restore PREPISUJE trenutnu konfiguraciju i baze podacima iz „'+n+'" te restarta servise (koristi se za oporavak uređaja nakon kvara). Traži ispravan ključ za dešifriranje na uređaju. Za potvrdu upiši VRATI:')!=='VRATI')return;const allR=document.querySelectorAll('.bkRestore');allR.forEach(b=>b.disabled=true);$('#bkMsg').textContent='Vraćam iz kopije… traje do minute, NE osvježavaj stranicu i ne klikći ponovno.';try{const r=await api('/api/backup/restore',{method:'POST',body:JSON.stringify({name:n})});$('#bkMsg').textContent=(r.message||'Restore dovršen.')+' Stranica se sama osvježava za 15 s…';setTimeout(()=>location.reload(),15000)}catch(e){$('#bkMsg').textContent=e.message;allR.forEach(b=>b.disabled=false)}});
const bkKI=$('#bkKeyImport');if(bkKI)bkKI.onclick=async()=>{const k=$('#bkKeyIn').value.trim();if(!k){$('#bkKeyMsg').textContent='Zalijepi svoj spremljeni ključ.';return}if(!confirm('Uvesti ovaj ključ kao ključ za dešifriranje backupa na OVOM uređaju? (koristi se pri oporavku na novom uređaju)'))return;$('#bkKeyMsg').textContent='Uvozim ključ…';try{const r=await api('/api/backup/key',{method:'POST',body:JSON.stringify({key:k})});$('#bkKeyMsg').textContent=r.message||'Ključ uvezen.';$('#bkKeyIn').value=''}catch(e){$('#bkKeyMsg').textContent=e.message}};
...
${help('<b>Način:</b> <b>Failover (pričuva)</b> = radi točno <b>jedna</b> veza; sav promet ide kroz <b>primarnu</b>, a kad ona padne (ne prolazi provjera dostupnosti) automatski se prebaci na <b>sekundarnu</b>, pa se vrati čim se primarna oporavi. <b>Balans (load-balance)</b> = promet se dijeli preko <b>svih ispravnih</b> veza istovremeno, po težini (brža veza dobije veći udio); uređaj sam pazi da odgovor izađe kroz istu vezu kroz koju je promet i ušao. <br><br>U failoveru <b>redoslijed = prioritet</b> (vrh = primarni; presloži strelicama), a težina se ignorira. <b>Provjera dostupnosti (health-check)</b> je IP koji uređaj povremeno pinga kroz svaku vezu da zna radi li ona. Zahtijeva <b>Gateway mod</b>. Primjena mijenja default rutu uz <b>120 s potvrdu</b> (auto-rollback ako izgubiš pristup). Preporuka: simuliraj ispad primarnog prije produkcije.')}
<label>Način rada <select id="muwMode"><option value="failover" ${fo?'selected':''}>Failover (pričuva) — radi jedna veza, ostale čekaju</option><option value="loadbalance" ${fo?'':'selected'}>Balans — promet preko svih veza istovremeno</option></select></label>
...
<div class="wizRow"><button id="vpnWiz">Dodaj VPN korisnika</button></div></div>`;
...
<div class="wizRow"><button id="pxWiz">Objavi servis (reverse proxy)</button></div>
...
runWizard('Objava servisa preko reverse proxyja',[
...
${d.installed===false?`<div class="panel error"><h2>Suricata nije instalirana</h2><p>Uređaj je instaliran bez IDS modula. Instalacija ne traži ponovnu instalaciju uređaja — dohvaća se paket i uključuje se ovdje.</p><button id="idsInstall">Instaliraj Suricatu</button> <span id="idsInstMsg" class="muted"></span></div>`:''}
<div class="wizRow"><button id="idsOn" ${d.hw.ok&&d.installed!==false?'':'disabled'}>Uključi IDS</button> <button id="ipsWiz" ${d.hw.ok?'':'disabled'}>Postavi IPS</button> <button id="idsUpd" class="ghost" title="Preuzmi najnoviju bazu prepoznavanja (suricata-update) i ponovno učitaj Suricatu">Ažuriraj pravila</button> <button id="idsOff" style="background:#e05d5d">Emergency IPS off</button></div>
...
const idsInstBtn=$('#idsInstall');if(idsInstBtn)idsInstBtn.onclick=async()=>{const m=$('#idsInstMsg');if(!confirm('Instalirati Suricatu? Preuzima se paket s Ubuntu repozitorija, može potrajati nekoliko minuta.'))return;idsInstBtn.disabled=true;m.textContent='Instaliram… (nekoliko minuta)';try{const r=await api('/api/ids/install',{method:'POST',body:'{}'});m.textContent=r.note||'Instalirano.';await idsPage()}catch(err){m.textContent=err.message;idsInstBtn.disabled=false}};
$('#idsUpd').onclick=async()=>{const b=$('#idsUpd');const m=$('#idsMsg');if(!confirm('Preuzeti najnoviju bazu prepoznavanja (suricata-update) i ponovno učitati Suricatu? Može potrajati do minute.'))return;b.disabled=true;m.textContent='Ažuriram bazu pravila… (može potrajati do minute)';try{await api('/api/ids/update-rules',{method:'POST',body:'{}'});m.textContent='Baza pravila ažurirana i Suricata ponovno učitana.'}catch(e){m.textContent=e.message}finally{b.disabled=false}};
...
runWizard('Uključivanje IPS zaštite',[
...
// Pristup uređaju: matrica zona × servisa (tko smije doći do samog uređaja).
// Redak WAN je posebno označen jer je upravljanje s te strane najčešća greška.
const ACLSVC=[['https','HTTPS (GUI)'],['ssh','SSH'],['dns','DNS'],['ping','Ping'],['dhcp','DHCP'],['vpn','VPN']];
async function deviceAccessPage(){const e=escapeHtml;const d=await api('/api/device-access');const zones=d.zones||[];let acls=d.acls||[];
if(!d.configured){$('#content').innerHTML='<div class="panel muted">Gateway još nije konfiguriran — postavi WAN i LAN u modulu Gateway.</div>';return}
const byZone=Object.fromEntries(acls.map(r=>[r.zone,r]));
const head=ACLSVC.map(([,l])=>`<th style="text-align:center">${l}</th>`).join('');
const rows=zones.map(z=>{const r=byZone[z.zone]||{};
const cells=ACLSVC.map(([k])=>`<td style="text-align:center"><input type="checkbox" data-z="${e(z.zone)}" data-s="${k}" ${r[k]?'checked':''}></td>`).join('');
return `<tr><td><b>${e(z.label)}</b>${z.untrusted?' <span class="status st-warn">nepovjerljiva</span>':''}<br><span class="muted">${e(z.interface)}</span></td>${cells}</tr>`}).join('');
$('#content').innerHTML=`<div class="panel"><h2>Pristup uređaju</h2>
<p class="muted">Označi koje servise samog uređaja smije koristiti koja zona. Prazna ćelija znači da promet iz te zone prema tom servisu firewall odbacuje.</p>
${d.derived?'<p class="status st-warn">Prikazano je stanje izvedeno iz trenutnih postavki. Spremanjem prelaziš na matricu i ona od tada određuje pristup.</p>':''}
${help('Ovo je pristup <b>samom uređaju</b> (upravljanje, DNS, DHCP), ne promet koji kroz njega prolazi — za to služe firewall pravila.<br><br><b>WAN redak je najvažniji:</b> ako su HTTPS ili SSH označeni, uređaju se može pristupiti s internetske strane. Za produkciju to isključi i upravljaj iz LAN-a ili preko VPN-a.<br><br>Uređaj neće dopustiti spremanje matrice u kojoj nijedna zona nema HTTPS ni SSH — time bi se izgubio svaki pristup. Nakon spremanja treba primijeniti firewall (Gateway → Primijeni), gdje vrijedi uobičajeni prozor od 120 s za potvrdu.')}
<div class="wizRow"><label>Administrativna mreža (neobavezno, dodatno ograničenje po izvorišnom CIDR-u) <input id="daAdmin" value="${e(d.adminNetwork||'')}" placeholder="npr. 192.168.10.0/24" style="min-width:16rem"></label></div>
<div style="overflow-x:auto"><table><thead><tr><th>Zona</th>${head}</tr></thead><tbody>${rows}</tbody></table></div>
<div class="wizRow"><button id="daSave">Spremi</button> <button id="daApply" class="ghost">Spremi i primijeni firewall</button></div>
<div id="daMsg" class="muted"></div></div>`;
const collect=()=>zones.map(z=>{const row={zone:z.zone};ACLSVC.forEach(([k])=>{const el=document.querySelector(`input[data-z="${CSS.escape(z.zone)}"][data-s="${k}"]`);row[k]=!!(el&&el.checked)});return row});
const save=async()=>{const r=await api('/api/device-access',{method:'PUT',body:JSON.stringify({acls:collect(),adminNetwork:$('#daAdmin').value.trim()})});return r};
$('#daSave').onclick=async()=>{const m=$('#daMsg');m.textContent='Spremam…';try{const r=await save();m.textContent=r.note||'Spremljeno.'}catch(err){m.textContent=err.message}};
$('#daApply').onclick=async()=>{const m=$('#daMsg');const wan=collect().find(r=>r.zone==='wan');
if(wan&&(wan.https||wan.ssh)&&!confirm('WAN redak dopušta upravljanje s internetske strane. Sigurno primijeniti?'))return;
m.textContent='Spremam…';try{await save();m.textContent='Primjenjujem firewall…';await api('/api/gateway/apply',{method:'POST',body:'{}'});
m.innerHTML='Primijenjeno. <b>Potvrdi unutar 120 s</b> ili se vraća prethodno stanje. <button id="daConfirm">Potvrdi (zadrži)</button> <button id="daRollback" class="ghost">Vrati odmah</button>';
$('#daConfirm').onclick=async()=>{try{await api('/api/gateway/confirm',{method:'POST',body:'{}'});m.textContent='Potvrđeno.'}catch(err){m.textContent=err.message}};
$('#daRollback').onclick=async()=>{try{await api('/api/gateway/rollback',{method:'POST',body:'{}'});m.textContent='Vraćeno prethodno stanje.'}catch(err){m.textContent=err.message}}}catch(err){m.textContent=err.message}}}
// Sigurnosni pregled: čita stanje otvrdnjavanja i nudi dva popravka koja traže
// root (SSH i jezgra). Ostale stavke vode u modul koji ih rješava.
async function hardeningPage(){const e=escapeHtml;const d=await api('/api/hardening');const cs=d.checks||[];const s=d.summary||{};
const icon=st=>st==='ok'?'<span class="status st-healthy">u redu</span>':st==='fail'?'<span class="status st-error">problem</span>':st==='warn'?'<span class="status st-warn">upozorenje</span>':'<span class="status st-muted">nepoznato</span>';
const rows=cs.map(c=>`<tr><td>${icon(c.status)}</td><td><b>${e(c.title)}</b></td><td>${e(c.detail)}</td><td>${c.fix&&c.status!=='ok'?`<button data-fix="${e(c.fix)}">Popravi</button>`:''}</td></tr>`).join('');
$('#content').innerHTML=`<div class="panel"><h2>Sigurnosni pregled</h2>
<p class="muted">U redu: <b>${s.ok||0}</b> · upozorenja: <b>${s.warn||0}</b> · problema: <b>${s.fail||0}</b>${s.unknown?` · neprovjerljivo: <b>${s.unknown}</b>`:''}</p>
${help('Provjerava je li uređaj stvarno postavljen onako kako bi trebao biti prije nego što se pusti u rad. <b>Popravi</b> postoji samo za dvije stavke koje traže root: SSH pristup i otvrdnjavanje mrežne jezgre. Ostalo se rješava u svom modulu (Gateway, IDS/IPS, SIEM, Backup).<br><br><b>Prije isključivanja SSH lozinki:</b> uređaj provjerava da <i>postoji</i> instaliran ključ, ali ne može znati imaš li ti njegov privatni par — ključ može biti i star, od nekoga tko ga više nema. Zato prvo sam probaj <code>ssh korisnik@adresa</code>: ako te pusti unutra <b>bez pitanja za lozinku</b>, popravak je siguran. Ako pita lozinku, prvo prebaci svoj ključ s <code>ssh-copy-id korisnik@adresa</code>. Ako se ipak zaključaš, s konzole uređaja vrati stanje s <code>sudo saguaro-harden ssh-revert</code>.')}
<div class="wizRow"><button id="hdRefresh" class="ghost">Osvježi</button></div>
<div id="hdMsg" class="muted"></div>
...
${help('<b>Ukratko:</b> filtrira i ubrzava web promet. Na uređaju se postavi <b>proxy = IP uređaja (na LAN-u) : port</b> (zadano 8080). Uređaj tada <b>kešira</b> (brže učitavanje čestih stranica) i <b>filtrira</b> koje se stranice smiju otvarati. <br><br><b>Dozvoljena mreža</b> = tko smije koristiti proxy. Filtriranje ide po <b>URL grupama</b> i <b>kategorijama</b> (blokirano/dozvoljeno). <br><br><b>SSL-bump</b> (napredno) otključava filtriranje <b>sadržaja HTTPS</b> stranica — uređaj dešifrira promet vlastitim certifikatom, pa ga klijenti <b>moraju imati u povjerenju</b>; osjetljive stranice (banke, zdravstvo) stavi u <b>Splice</b> listu da se NE dešifriraju (privatnost/zakon). <br><br>Savjet: za jeftino blokiranje po domeni koristi i <b>DNS filtering</b> — proxy dodaje keš i kontrolu po pojedinom URL-u. <br><i>Tehnički:</i> Squid + e2guardian.')}</div>
...
$('#content').innerHTML=`${tabBar('dnsTabs',[['zone','Zone i zapisi',zones.length],['split','Split-horizon',split.length],['upstream','Šifrirani upstream']])}
...
<div class="tabpane" data-pane="dnsTabs" data-tabkey="upstream"><div id="fwdPane"></div></div>`;
wireTabs('dnsTabs');
...
api('/api/dns/forward').then(c=>renderDnsForward({enabled:!!c.enabled,upstreams:(c.upstreams||[]).map(u=>({...u}))})).catch(()=>{});
if(dnsZone)await openZone(dnsZone)}
// renderDnsForward draws the encrypted (DoT) upstream editor into #fwdPane.
const DOT_PRESETS={cloudflare:{label:'Cloudflare',ups:[{address:'1.1.1.1',port:853,name:'cloudflare-dns.com'},{address:'1.0.0.1',port:853,name:'cloudflare-dns.com'}]},quad9:{label:'Quad9',ups:[{address:'9.9.9.9',port:853,name:'dns.quad9.net'},{address:'149.112.112.112',port:853,name:'dns.quad9.net'}]},google:{label:'Google',ups:[{address:'8.8.8.8',port:853,name:'dns.google'},{address:'8.8.4.4',port:853,name:'dns.google'}]}};
function renderDnsForward(cfg){const e=escapeHtml;const el=$('#fwdPane');if(!el)return;
const rows=cfg.upstreams.length?cfg.upstreams.map((u,i)=>`<tr><td>${e(u.address)}<span class="muted">@${u.port||853}</span></td><td class="muted">${e(u.name||'—')}</td><td class="rowacts">${iconBtn('del','Ukloni','danger fwdDel',`data-i="${i}"`)}</td></tr>`).join(''):'<tr><td colspan="3" class="muted">Nema upstreamova — odaberi providera ili dodaj ručno.</td></tr>';
el.innerHTML=`<div class="panel"><h2>Šifrirani DNS upstream (DoT)</h2>
${help('Uređaj po defaultu <b>sam rekurzivno</b> rješava internet DNS (Unbound → root serveri). Ovdje umjesto toga možeš cijelu rekurziju <b>proslijediti šifrirano (DNS-over-TLS, port 853)</b> na povjerljivog providera (Cloudflare, Quad9, Google) ili vlastiti — tako ISP ne vidi koje domene tražiš. TLS certifikat upstreama se provjerava (<i>tls-cert-bundle</i>). <br><br><i>Napomena:</i> Unbound je nativni <b>DoT</b> klijent; DoH (preko HTTPS-a) Unbound ne podržava kao upstream. Isključeno = uređaj ostaje puni rekurzivni resolver (preporuka za maksimalnu neovisnost).')}
${toggle('fwdEnabled',cfg.enabled,'Uključi šifrirani upstream (isključeno = vlastita rekurzija)')}
<div class="btnrow" style="margin:.3rem 0"><span class="muted small" style="align-self:center">Predlošci:</span>${Object.entries(DOT_PRESETS).map(([k,p])=>`<button type="button" class="ghost fwdPreset" data-k="${k}">${e(p.label)}</button>`).join(' ')} <button type="button" class="ghost" id="fwdAdd">+ Dodaj ručno</button></div>
<table><thead><tr><th>Adresa</th><th>TLS ime (provjera certa)</th><th></th></tr></thead><tbody>${rows}</tbody></table>
<div class="btnrow" style="margin-top:.5rem"><button type="button" id="fwdApply">Primijeni</button></div><div id="fwdMsg" class="muted"></div></div>`;
$('#fwdEnabled').onchange=()=>{cfg.enabled=$('#fwdEnabled').checked};
document.querySelectorAll('.fwdPreset').forEach(b=>b.onclick=()=>{const p=DOT_PRESETS[b.dataset.k];cfg.upstreams=p.ups.map(u=>({...u}));cfg.enabled=true;renderDnsForward(cfg)});
document.querySelectorAll('.fwdDel').forEach(b=>b.onclick=()=>{cfg.upstreams.splice(+b.dataset.i,1);renderDnsForward(cfg)});
$('#fwdAdd').onclick=()=>{const address=(prompt('IP adresa upstream resolvera:','')||'').trim();if(!address)return;const port=parseInt(prompt('Port (DoT je 853):','853'),10)||853;const name=(prompt('TLS ime za provjeru certifikata (npr. dns.quad9.net; prazno = bez provjere imena):','')||'').trim();cfg.upstreams.push({address,port,name});renderDnsForward(cfg)};
$('#fwdApply').onclick=async()=>{const m=$('#fwdMsg');if(cfg.enabled&&!cfg.upstreams.length){m.textContent='Dodaj barem jedan upstream ili isključi.';return}if(cfg.enabled&&!confirm('Proslijediti SVU DNS rekurziju šifrirano na odabrane upstreamove? Uređaj više neće sam rješavati preko root servera.'))return;m.textContent='Primjena…';try{const r=await api('/api/dns/forward',{method:'POST',body:JSON.stringify({enabled:cfg.enabled,upstreams:cfg.upstreams})});m.textContent=cfg.enabled?('Primijenjeno — '+(r.upstreams||cfg.upstreams.length)+' upstream(ova) preko DoT-a.'):'Isključeno — uređaj opet sam rekurzivno rješava.'}catch(err){m.textContent=err.message}}}
...
function wizDhcpNet(){runWizard('Nova DHCP mreža',[
...
runWizard('Nova DHCP rezervacija',[
...
function wizDnsZone(){runWizard('Nova DNS zona',[
