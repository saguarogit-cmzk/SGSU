...
document.querySelectorAll('.bkRestore').forEach(el=>el.onclick=async()=>{const n=el.dataset.n;if(prompt('⚠ OPREZ: restore PREPISUJE trenutnu konfiguraciju i baze podacima iz „'+n+'" te restarta servise (koristi se za oporavak uređaja nakon kvara). Traži ispravan ključ za dešifriranje na uređaju. Za potvrdu upiši VRATI:')!=='VRATI')return;$('#bkMsg').textContent='Vraćam iz kopije… servisi se restartaju, GUI se može nakratko prekinuti.';el.disabled=true;try{const r=await api('/api/backup/restore',{method:'POST',body:JSON.stringify({name:n})});$('#bkMsg').textContent=r.message||'Restore dovršen.'}catch(e){$('#bkMsg').textContent=e.message;el.disabled=false}});
