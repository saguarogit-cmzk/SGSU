...
const rows=cfg.uplinks.length?cfg.uplinks.map((u,i)=>{const prio=fo?`<td><b>${i===0?'Primarni':i===1?'Sekundarni':'Pričuva '+(i+1)}</b> <span class="muted small">#${i+1}</span></td>`:`<td>${u.weight}</td>`;const reorder=fo?`${iconBtn('up','Prioritet gore','muwUp',`data-i="${i}"${i===0?' disabled':''}`)}${iconBtn('down','Prioritet dolje','muwDown',`data-i="${i}"${i===cfg.uplinks.length-1?' disabled':''}`)}`:'';return `<tr><td><b>${e(u.name)}</b></td><td class="muted">${e(nlabel(u.interface))}</td><td class="muted">${e(u.gateway)}</td>${prio}<td class="muted">${e(u.healthCheck||'—')}</td><td class="rowacts">${reorder}${iconBtn('del','Ukloni','danger muwDel',`data-i="${i}"`)}</td></tr>`}).join(''):'<tr><td colspan="6" class="muted">Nema uplinkova — dodaj barem dva.</td></tr>';
...
<div class="panel"><h2>Multi-WAN ${m.enabled?'<span class="badge">aktivan</span>':''}${m.enabled?` <span class="badge">${e(m.mode||'loadbalance')}</span>`:''}</h2>
...
<table><thead><tr><th>Naziv</th><th>Interface</th><th>Gateway</th><th>${fo?'Prioritet':'Težina'}</th><th>Health-check</th><th></th></tr></thead><tbody>${rows}</tbody></table>
...
const ap=$('#wanApply');if(ap)ap.onclick=async()=>{if(cfg.uplinks.length<2){$('#wanMsg').textContent='Treba barem dva uplinka.';return}if(!confirm('Primijeniti multi-WAN ('+cfg.mode+')? Mijenja default rutu. MORAŠ potvrditi unutar 120 s inače se vraća staro (zaštita od gubitka pristupa).'))return;$('#wanMsg').textContent='Primjena…';try{await api('/api/multiwan/apply',{method:'POST',body:JSON.stringify({enabled:true,mode:cfg.mode,uplinks:cfg.uplinks})});multiwanPage()}catch(err){$('#wanMsg').textContent=err.message}};
const off=$('#wanOff');if(off)off.onclick=async()=>{if(!confirm('Isključiti multi-WAN?'))return;try{await api('/api/multiwan/apply',{method:'POST',body:JSON.stringify({enabled:false,mode:cfg.mode,uplinks:cfg.uplinks})});multiwanPage()}catch(err){alert(err.message)}};
...
<div class="panel"><div class="btnrow" style="justify-content:space-between;align-items:center"><h2 style="margin:0">Zone (${zones.length})</h2><button type="button" id="zoneNew">+ Dodaj zonu</button></div>
...
$('#zoneDetail').innerHTML=`<div class="panel"><div class="btnrow" style="justify-content:space-between;align-items:center"><h2 style="margin:0">Zapisi — ${escapeHtml(name)}</h2><button type="button" id="recNew">+ Dodaj zapis</button></div>${sets.length>8?searchBar('recSearch','Traži zapis po imenu / vrijednosti…'):''}<table><thead><tr><th>Ime</th><th>Tip</th><th>TTL</th><th>Vrijednosti</th><th></th></tr></thead><tbody id="recTbody">${sets.map((s,i)=>`<tr><td>${escapeHtml(s.name)}</td><td>${escapeHtml(s.type)}</td><td>${s.ttl}</td><td>${escapeHtml((s.records||[]).map(r=>r.content).join(', '))}</td><td><button class="recEdit" data-i="${i}">Uredi</button> <button class="recDel" data-n="${escapeHtml(s.name)}" data-t="${escapeHtml(s.type)}">Obriši</button></td></tr>`).join('')}</tbody></table>
