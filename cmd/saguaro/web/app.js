...
const $=s=>document.querySelector(s); let current='dashboard'; let dashTimer=null; let lastMetric=null;
...
const NAV_GROUPS=[
 ['Status',['dashboard','monitoring','conflicts','audit']],
 ['Mreža',['interfaces','gateway','routing','multiwan']],
 ['DNS i DHCP',['dns','dhcp','rpz']],
 ['Vatrozid',['fwrules','ids']],
 ['VPN',['vpn','sitevpn','ipsec']],
 ['Servisi i TLS',['certificates','proxy','services','backup','mail']],
 ['Sustav',['system','users']],
];
...
<div class="panel"><h2>Komponente</h2><div class="services">${services.map(s=>`<div class="card service"><div><h3>${escapeHtml(s.name)}</h3><p>${escapeHtml(s.description)}</p></div><div><span class="status ${stClass(s.status)}">${escapeHtml(s.status)}</span><button onclick="checkService('${s.id}')">Provjeri</button></div></div>`).join('')}</div></div>`;
...
lastMetric=null;await refreshMetrics();dashTimer=setInterval(refreshMetrics,2000)}
