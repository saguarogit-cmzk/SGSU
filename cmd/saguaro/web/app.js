...
${help('Ovdje se ažurira <b>sam Saguaro Network Manager</b> (control plane + adapteri + sudoers), ne samo OS servisi. Kod git-instalacije <b>Ažuriraj iz gita</b> radi <code>git fetch</code> + hard-reset na <code>origin/main</code>, ponovno gradi binarije, osvježava adaptere i sudoers (uz validaciju) i restarta servis (~par sekundi prekida GUI-ja). Tako sve što promijenimo na gitu možeš kontrolirano povući na uređaj. Smije samo <b>admin</b>.')}
${su.gitRepo?`<div class="wizRow">${admin?`<button id="suApply" ${suBehind?'':'class="ghost"'}>Ažuriraj Saguaro iz gita${suBehind?' ('+su.behind+')':''}</button>`:'<span class="muted">Samo admin može ažurirati control plane.</span>'}${su.buildable===false?'<span class="muted"> — Go toolchain nije dostupan, build nije moguć.</span>':''}</div><div id="suMsg" class="muted"></div>`:''}</div>`;
...
const suBtn=$('#suApply');if(suBtn)suBtn.onclick=async()=>{if(!confirm('Ažurirati Saguaro control plane iz gita? Povlači origin/main, gradi binarije i restarta servis (~par sekundi prekida GUI-ja).'))return;const msg=$('#suMsg');suBtn.disabled=true;msg.textContent='Povlačim i gradim iz gita… (može potrajati minutu)';try{const r=await api('/api/selfupdate/apply',{method:'POST',body:'{}'});msg.textContent=(r.message||'Ažurirano')+' — GUI se restarta, osvježi stranicu za par sekundi.'}catch(err){msg.textContent=err.message;suBtn.disabled=false}};
