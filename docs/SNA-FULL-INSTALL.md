...
| — | **Status 2026-07-18:** S1–S9 riješeni. S5: CSRF token vezan uz sesiju (double-submit; `saguaro_csrf` kolačić + `X-CSRF-Token` header, SHA-256 hash u session zapisu, provjera u auth middlewareu za sve ne-GET zahtjeve). S6: eksponencijalni lockout po IP-u i po korisničkom imenu (5 slobodnih pokušaja, zatim 30 s → ×2 do 15 min; 429 + `Retry-After`; audit događaj `login-lockout` sa severity `security`). S7: GitHub Actions (`.github/workflows/build-deb.yml`) builda `saguaro_*.deb` za amd64+arm64 (`scripts/build-deb.sh`, maintainer skripte u `packaging/deb/`); installer v0.4.0 prima `--deb PATH\|URL` i tada ne instalira golang-go niti builda na uređaju (source-build ostaje fallback). S10: pravi dubinski health checkovi u control planeu (`health.go`): PostgreSQL ping, TCP proba resolvera, PowerDNS API (X-API-Key), Kea `status-get` za dhcp4 i d2 (basic auth), systemd stanja za nginx/nftables/step-ca; pozadinski sweep svakih 5 min + autenticirani `GET /api/health/deep` + POST `check` akcija po servisu; `/api/health` ostaje neautenticirani liveness. **Svi nalazi S1–S10 riješeni.** Event collector implementiran (v0.5.0): `saguaro-eventd` (journald `-o json` → PG `events` po shemi iz Faze 1, mjesečne particije + retencija DROP PARTITION kroz `SAGUARO_EVENT_RETENTION_MONTHS`, batch insert, cursor-resume, min severity `notice`); `audit_log` kreira installer kao postgres s INSERT/SELECT-only grantovima (append-only na razini baze), control plane u njega dual-writea audit + security evente u `events`; GUI Monitoring prikazuje `GET /api/events`. Mail modul implementiran (v0.6.0): `internal/mail` — SMTP (STARTTLS/TLS/plain, std lib bez novih ovisnosti), lozinka AES-256-GCM šifrirana ključem iz `/etc/saguaro/secret.key` (root:saguaro 0640, installer generira; API vraća samo `hasPassword`), agregator "prvi event odmah, duplikati 10 min, zatim sažetak"; alert petlja u control planeu čita `events` ≥ `minSeverity` svakih 30 s (kursor od zadnjeg ID-a — bez alarma na povijest); GUI modul Mail (W10 tok: postavke → test mail → uključi), API GET/PUT `/api/mail` + POST `/api/mail/test`. DHCP/DNS adapteri implementirani (v0.7.0): DNS CRUD kroz PowerDNS API (`internal/adapters/pdns` — zone create/delete, rrset REPLACE/DELETE, allowlist tipova u GUI-ju, DNSSEC tipovi skriveni); DHCP kroz Kea control agent (`internal/adapters/kea` — status-get, config-get→subneti, lease4-get-all preko `lease_cmds` hooka koji installer sada učitava) + **rezervacije izravno u Kea PG `hosts` tablici** (bez premium hookova; installer daje GRANT roli saguaro, `SAGUARO_KEA_DB_DSN` u env). GUI stranice DNS (zone+zapisi) i DHCP (subneti, leaseovi, rezervacije). RBAC implementiran (v0.8.0): uloge admin/network-operator/dns-operator/auditor/read-only iz specifikacije; user store u PG (`users` tablica) ili file (dev), bootstrap admin seeda se iz LoadCredential lozinke samo dok je store prazan; čitanja dopuštena svim prijavljenima, mutacije po permission tablici (`authz.go`); uloga se čita **živo** po zahtjevu pa disable/promjena uloge ruši postojeće sesije odmah; odbijeni pokušaji = security audit `authz-deny`; zaštita zadnjeg aktivnog admina; self-service promjena lozinke uz staru; GUI Users stranica (upravljanje + moj račun). Subnet CRUD implementiran (v0.9.0): **bez root agenta** — Kea core API komande pokrivaju cijelu transakciju iz specifikacije: `config-get` (inspect) → mutacija subnet4 liste → `config-test` (validate) → `config-set` (apply) → `status-get` (verify, uz automatski `config-set` rollback na staru konfiguraciju) → `config-write` (persist; installer zato daje `_kea:_kea` vlasništvo nad kea-dhcp4.conf) + JSON backup u /var/lib/saguaro. Validacije: CIDR, pool unutar subneta i uredan redoslijed, router unutar subneta; CIDR se ne mijenja (delete+add); brisanje subneta s rezervacijama traži `?force=true`; update čuva ostala per-subnet polja (rezervacije, klase, timere). API POST/PUT/DELETE `/api/dhcp/subnets`, GUI forma s uredi/obriši. Wizardi (v0.10.0): **W1** obavezna promjena bootstrap lozinke — `must_change` flag na korisniku (seedani admin, admin-kreirani korisnici i reset lozinke ga postavljaju), middleware blokira sve osim profila/promjene/odjave dok se ne promijeni, GUI forsira modal nakon prijave; **W2** DHCP mreža (koraci mreža→pool/opcije→pregled; server odbija preklapanje CIDR-ova); **W3** DHCP rezervacija (odabir iz leaseova ili ručno; upozorenje za IP unutar poola; server odbija duplikat MAC/IP); **W4** DNS zona (forward/reverse, reverse ime automatski iz /24 subneta). Wizard okvir (`runWizard`) + panel Čarobnjaci na dashboardu (W2/W3/W4/W10). W5–W7, W11–W12 čekaju module (proxy, VPN, backup ciljevi). Gateway mod + W8 implementiran (v0.11.0): `internal/adapters/nftgen` generira kompletan nftables.conf (host firewall + u gateway modu forward lanac, dnat port forwardi, masquerade; validacije: CIDR-ovi, imena sučelja, WAN≠LAN, zabrana forwarda mgmt portova 22/443); **root adapter `/usr/sbin/saguaro-firewall`** (jedina sudo dozvola za saguaro, allowlist check/apply/confirm/rollback) radi točan tok iz specifikacije: nft -c validacija → backup → primjena → **systemd-run tajmer vraća staru konfiguraciju za 120 s ako nema potvrde** → sysctl ip_forward po potrebi; API `/api/gateway` (get/put/preview/apply/confirm/rollback, nova rola-dozvola `firewall:write` za admin+network-operator, sve security audit); GUI Gateway stranica (NIC-ovi, forma, pregled pravila, pending banner s Potvrdi/Vrati) + W8 wizard s preduvjetom 2+ NIC-a. Suricata W9 implementiran (v0.12.0): `internal/adapters/suricata` generira suricata.yaml (af-packet IDS ili NFQUEUE IPS s `fail-open: yes`); root adapter `/usr/sbin/saguaro-ids` (sudoers allowlist `apply ids|ips`, `disable`, `update-rules`; `suricata -T` validacija + systemd override); eve.json alerti idu kroz eventd tailer (rotacija-safe) u `events` (modul ids; Suricata sev 1→security → mail alarmi automatski); **hardverski gate ≥8 GB/4 jezgre nametnut i u API-ju i u installeru** (`--with-suricata` odbija low-spec); IPS preduvjeti: gateway mod + IDS period promatranja (`SAGUARO_IPS_MIN_IDS_DAYS`, default 7; force uz audit); IPS uključuje nftables `ct state new queue num 0 bypass` kroz postojeću 120 s firewall transakciju; "Emergency IPS off" bez preduvjeta; GUI IDS/IPS stranica + W9 wizard (statistika 14 d → zona → drop policy → apply). RPZ filtering tier implementiran (v0.13.0): `internal/adapters/rpz` generira RPZ zonu (domena + poddomene → NXDOMAIN preko `CNAME .`, dedup, validacija domena) i Unbound drop-in (`module-config respip`, `rpz-action-override: nxdomain`, `rpz-log`, opcijski vanjski feedovi kroz `url:`); root adapter `/usr/sbin/saguaro-rpz` (sudoers `apply`/`disable`; `unbound-checkconf` validacija s automatskim revertom drop-ina na grešku); **bez hardverskog gatea — ovo je security razina za profil A**; RPZ pogoci se iz unbound logova promoviraju u `dns-filter` evente (akcija `rpz-block`, min. notice) pa ulaze u Monitoring i mail alerting; API GET `/api/rpz` + POST `/api/rpz/apply` (uloga `dns:write`), GUI stranica DNS filtering. W5 publish service implementiran (v0.14.0): `internal/adapters/nginxgen` generira vhostove objavljenih servisa (TLS appliance/custom/none, allow/deny access politika po CIDR-ima, WebSocket podrška, dedup imena i hostnamea, validacije protiv injectiona); root adapter `/usr/sbin/saguaro-proxy` (sudoers `apply`/`disable`; instalacija u sites-available + symlink, **`nginx -t` validacija s automatskim revertom na prethodnu konfiguraciju**, reload); **upstream TCP reachability proba prije objave** (409 + `force=true` override); API GET `/api/proxy` + POST `/api/proxy/apply` (nova dozvola `proxy:write`, admin+network-operator); GUI Reverse Proxy stranica + W5 wizard (servis→upstream→hostname→TLS→pristup→pregled) s **automatskom ponudom DNS A zapisa** za hostname. ACME automatizacija (step-ca/LE) dolazi s W6. W6 certifikati implementirani (v0.15.0): interni certifikati iz Step CA appliancea kroz root adapter `/usr/sbin/saguaro-cert` (sudoers `issue`/`renew-all`/`remove *`/`deploy-gui *`; `step ca certificate` s provisionerom admin, rok 90 dana, ključ 0600 root); **dnevni renewal timer** (`saguaro-cert-renew.timer`, `step ca renew` + nginx reload); deploy ciljevi: GUI vhost (zamjena bootstrap certifikata!) i objavljeni servisi kroz novi TLS izvor **`managed`** u nginxgen (putanje /etc/saguaro/certs/<ime>.{crt,key}); brisanje certifikata blokirano dok ga referencira objavljeni servis; API `/api/certs` (list s živim NotAfter iz .crt, issue, deploy-gui, delete; dozvola `certs:write` samo admin); GUI Certificates stranica + W6 wizard (tip → ime/SAN-ovi → deploy cilj → izdavanje); javni LE certifikati vraćaju 501 s objašnjenjem (politika: javni cert za privatna imena zabranjen). W7 VPN implementiran (v0.16.0): `internal/adapters/wireguard` — generacija ključeva u procesu (curve25519 iz već pinanog x/crypto; **klijentski privatni ključ postoji samo u trenutku kreiranja i nikad se ne pohranjuje**), serverski privatni ključ zapečaćen AES-256-GCM ključem appliancea, jedinstvena alokacija peer adresa (najniža slobodna, reuse rupa), server conf isključuje istekle peerove (rok valjanosti), klijentski conf s DNS pushom i split (VPN subnet + lokalne mreže) ili full tunelom; root adapter `/usr/sbin/saguaro-vpn` (apply s restore-om na neuspjeli `wg-quick` start, disable); QR kod server-side (pure-Go go-qrcode, bez tranzitivnih ovisnosti) + .conf download; API `/api/vpn` (apply/peers, dozvola `firewall:write`); installer dodaje `wireguard-tools`; GUI VPN stranica + W7 wizard (korisnik → profil → rok → jednokratni prikaz QR/conf). W11 backup + W12 Multi-WAN implementirani (v0.17.0) — **svih 12 wizarda pokriveno**. W11: `saguaro-backup.sh` proširen za SFTP (scp) i S3 (aws cli) upload šifriranih arhiva; root adapter `/usr/sbin/saguaro-backup-config` (apply piše backup.env + timer OnCalendar drop-in, run = backup odmah, disable); tajne (SFTP ključ/S3 secret) zapečaćene AES-GCM ključem appliancea; **restore drill**: `LastDrill` u stanju, dnevna provjera emitira `warning` event `restore-drill-overdue` nakon 90 dana, GUI banner + "Označi drill izvedenim"; API `/api/backup` (apply/run/drill, dozvola `backup:write` admin). W12: `internal/adapters/multiwan` (ponderirani multipath default route, spec datoteka); root adapter `/usr/sbin/saguaro-wan` (apply gradi rutu + uključuje `saguaro-wan-check.timer` koji svakih ~20 s ping health-checkom vraća/uklanja nexthope, disable); zahtijeva Gateway mod; API `/api/multiwan` (dozvola `firewall:write`); GUI stranice Backup i Multi-WAN + W11/W12 tokovi. | `scripts/saguaro-backup*`, `internal/adapters/multiwan/`, `cmd/saguaro/{backup,multiwan}.go` | — | — |
...
| SR2 | Nizak | `saguaro-firewall`/`saguaro-ids` validiraju **staged** datoteku pa je instaliraju (TOCTOU: učitana ≠ validirana). Utjecaj ograničen — nevaljana konfiguracija ne učita se, a valjana je unutar ovlasti modula. | **RIJEŠENO (v0.17.2)**: oba adaptera sada instaliraju u odredište (root-owned `/etc`) pa validiraju odredište (`nft -c -f`/`suricata -T`) i vraćaju backup na grešku — validira se i učitava ista datoteka koju saguaro ne može zamijeniti. |
